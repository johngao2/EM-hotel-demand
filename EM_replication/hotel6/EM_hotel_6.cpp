...
#define n_times 10 // number of time steps
#define n_options 3 // number of products
#define n_types 4  // number of customer types
...
int **import_prefs(const char *pref_filename)
{
	...
	io::CSVReader<5> in(pref_filename);
...
{
	io::CSVReader<4> in(avail_filename);
	...
	while (in.read_row(t, prod[0], prod[1], prod[2]))
	{
...
int main()
{
	...
	int **sigma_matrix = import_prefs("../data/hotel_6/PrefListsBuyUpH6.csv");
	int **avail_matrix = import_availability("../data/hotel_6/AvailabilityH6.csv");
	int *trans_vec = import_transactions("../data/hotel_6/TransactionsH6.csv");
	int **mu_matrix = build_mu_mat(sigma_matrix, avail_matrix, trans_vec);
	// Data import debugging prints ##################################################
	{
		printMatrix("PREFERENCE MATRIX:", sigma_matrix, n_types, n_options + 1, 3);
		printMatrix("AVAILABILITY MATRIX:", avail_matrix, n_times, n_options, 3);
		printVector("TRANSACTION VECTOR:", trans_vec, n_times, 3);
		printMatrix("MU MATRIX:", mu_matrix, n_times, n_types, 3);
	}
