...
int **import_prefs(const char *pref_filename, int verbose = 0) {
  ...
  bool viable; // tracks if an option is viable (i.e. ranked lower than
               // nonpurchase)
  ...
  if (!is_file_exist(filename.c_str())) {
    ...
    for (int t = 0; t < n_times; t++) {
      ...
      for (int i = 0; i < n_types; i++) // iterate over each cust type
      {
        ...
        {
          bool available = avail_matrix[t][k - 1]; // need to subtract 1 from k due to
                                                   // 1-indexed sigma matrix
...
int main() {
  ...
  // ##################################################
  {
    ...
    printMatrix("MU MATRIX:", mu_matrix, 10, 10, 3);
