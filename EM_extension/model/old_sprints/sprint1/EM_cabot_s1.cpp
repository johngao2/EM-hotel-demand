...
void printMatrix(const char *text, T mat, std::size_t N, std::size_t M, int width) {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < M; j++)
      std::cout << std::setw(width) << std::fixed << std::setprecision(2) << mat[i][j]
                << std::setw(width);
    std::cout << std::endl;
  }
  std::cout << std::endl;
...
void printVector(const char *text, T mat, std::size_t N, int width, int precision = 2) {
  ...
  for (int i = 0; i < N; i++) {
    std::cout << std::setw(width) << std::fixed << std::setprecision(precision) << mat[i]
              << std::setw(width);
    std::cout << std::endl;
  }
  std::cout << std::endl;
...
int **import_prefs(const char *pref_filename) {
  ...
  in.read_header(io::ignore_no_column, "cust_type", "rank_1", "rank_2", "rank_3", "rank_4",
  ...
  while (in.read_row(cust_type, pref_rank[0], pref_rank[1], pref_rank[2], pref_rank[3],
                     pref_rank[4], pref_rank[5], pref_rank[6], pref_rank[7])) {
...
int **import_availability(const char *avail_filename) {
  ...
  in.read_header(io::ignore_no_column, "T", "prod_1", "prod_2", "prod_3", "prod_4", "prod_5",
                 "prod_6", "prod_7");
  int t;
  int prod[n_options];
  int row_counter = 0;
  ...
  while (in.read_row(t, prod[0], prod[1], prod[2], prod[3], prod[4], prod[5], prod[6])) {
  ...
  if (!is_file_exist("mu_matrix.csv")) {
    ...
    for (int t = 0; t < n_times; t++) {
      ...
      for (int i = 0; i < n_types; i++) // iterate over each cust type
      {
        ...
        {
          bool available =
              avail_matrix[t][k - 1];     // need to subtract 1 from k due to 1-indexed sigma matrix
...
public:
  ...
  void operator()(ADvector &fg, const ADvector &x) {
    ...
    fg[0] += (n_purch + sum_ambiguous_a) * log(x[n_types]) +
             ((n_times - n_purch) - sum_ambiguous_a) * log(1 - x[n_types]);
